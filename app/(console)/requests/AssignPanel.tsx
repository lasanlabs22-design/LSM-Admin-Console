...
import { WORK_STATUS, roleMeta, tint } from '@/lib/meta';
import type { AssignData, Vendor } from '@/lib/types';
...
export default function AssignPanel({
  ...
  const chosen = (data?.vendors || []).filter((v) =>
    picked.some((n) => n.toLowerCase() === (v.name || '').toLowerCase())
  );

  const others = (data?.vendors || []).filter((v) => !chosen.includes(v));

  return (
    <Dialog
      label="Assign this work"
      size="panel"
          ...
          <div>
            ...
            <div className="space-y-2">
              {data!.assignments.map((a) => {
                const s = WORK_STATUS[a.status] || WORK_STATUS.offered;
...
function PartnerRow({
  ...
  const roleColour = roleMeta(v.role).color;
